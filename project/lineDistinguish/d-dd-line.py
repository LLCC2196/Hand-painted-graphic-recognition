...
def lineDerivative(line,step=1,averstep=5):
	dline = []
...
def plotLineproperties(img,contour,dline,ddline,amplitude = 10,x_axis = 400,y_axis = 400):
	...
	plt.subplot(2,2,1),plt.title('drawcontour')
	plt.imshow(img)
...
img = cv2.imread('line6.jpeg',0)
imgori = cv2.imread('line6.jpeg')
contour = lineContours(img)
dline = lineDerivative(contour)
ddline = lineCurvature(dline)
plotLineproperties(imgori,contour,dline,ddline)
